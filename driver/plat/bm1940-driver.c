...
int bm1940_pack_cmd_frame(uint8_t *str)
{
    ...
    frame->header_55 = BM1940_HEADER_55;
    frame->header_aa = BM1940_HEADER_AA;
    frame->chksum    = 0;
    uint16_t len = frame->data_len + BM1940_CMD_HEADER_LEN;
...
int bm1940_parse_respond_len(uint8_t *str, int len, int *read_len, int *st)
{
    ...
    switch(state)
    {
        case SEARCH_0XAA:
            *read_len = 1;
            if (len > 0 && str[0] == BM1940_HEADER_AA) {
                state = SEARCH_0X55;
                ret = PKG_PARSE_MIDDLE_STATE;
            }
            break;
        case SEARCH_0X55:
            if (len > 0 && str[0] == BM1940_HEADER_55) {
            ...
            *read_len = BM1940_ACK_HEADER_LEN - 2;
            break;
...
int bm1940_parse_respond_pkg(uint8_t *str, uint32_t len, int *type, uint8_t *out_str, uint32_t out_len)
{
    ...
    if ((frame->header_aa != BM1940_HEADER_AA) || (frame->header_55 != BM1940_HEADER_55))
      return 0;
...
int bm1940_pack_ioctl_pkg(uint8_t *str, uint32_t str_len, uint32_t oper_type, void *param)
{
    ...
    frame->cmd = oper_type;
    memcpy(str, param, frame->data_len + BM1940_CMD_HEADER_LEN);
    return bm1940_pack_cmd_frame(str);
}

int bm1940_soc_init(void *arg)
{
    struct chip_info *chip = (struct chip_info *)arg;
    chip->work_len  = BM1940_WORK_LEN;
    chip->nonce_len = BM1940_RESP_NONCE_LEN;
    chip->reg_len   = BM1940_RESP_REG_LEN;
    chip->frame_len = BM1940_DATAFRAME_LEN;
